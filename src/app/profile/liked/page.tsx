...
export default async function LikedReelsPage() {
  ...
  // Fetch all liked reels for the user
  ...
  const likedReelIds = userLikes?.map(like => like.reel_id) || [];

  let likedReels: any[] = [];
  if (likedReelIds.length > 0) {
    const { data: likedReelsData } = await supabaseClient
      .from('reels')
      .select('*')
      .in('id', likedReelIds);
      
    // Sort the fetched reels to match the order of liked reelIds
    if (likedReelsData) {
      likedReels = likedReelsData.sort((a, b) => {
        return likedReelIds.indexOf(a.id) - likedReelIds.indexOf(b.id);
      });
    }
  }
      ...
      <div className="max-w-4xl mx-auto space-y-10 pb-20 mt-4 md:mt-0">
        ...
        <section className="flex flex-col md:flex-row items-center justify-between gap-4 md:gap-8 bg-white/5 border border-white/10 py-6 px-5 md:p-8 rounded-2xl md:rounded-3xl backdrop-blur-md">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-pink-500/20 flex items-center justify-center border border-pink-500/30 shrink-0">
              <History className="w-5 h-5 md:w-8 md:h-8 text-pink-400" />
            </div>
            <div>
              <h1 className="text-xl md:text-4xl font-bold tracking-wider">All Liked Reels</h1>
              <p className="text-white/50 text-sm md:text-base mt-0.5 md:mt-1">{likedReels.length} reels liked</p>
            </div>
          </div>
        </section>
        ...
        <section>
          {likedReels.length > 0 ? (
            ...
            <div className="text-center py-16 bg-white/5 rounded-3xl border border-white/10">
              ...
              <p className="text-white/50 text-lg">You haven&apos;t liked any reels yet.</p>
